...
export default function TopBar({ onOpenMobileNav }: { onOpenMobileNav: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center gap-3 h-[64px] px-4 lg:px-8 border-b border-white/[0.06] bg-ink-950/70 backdrop-blur-xl">
      ...
      <div className="flex-1 max-w-md hidden sm:flex items-center gap-2 px-3 h-9 rounded-xl btn-ghost">
      ...
      <button className="relative p-2 rounded-lg btn-ghost">
        <Bell className="w-[18px] h-[18px] text-bone-300" />
        <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 rounded-full bg-fission-400" />
      </button>
